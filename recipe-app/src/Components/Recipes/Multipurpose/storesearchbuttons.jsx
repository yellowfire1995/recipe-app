export default function StoreButtons(props) {
  const ingredient = props.ingredient;
  return (
    <>
      <a
        href={`https://www.walmart.com/search?q=${ingredient}`}
        target="_blank"
        rel="noopener noreferrer"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          enableBackground="new 0 0 40 40"
          viewBox="0 0 40 40"
          id="walmart"
          height="25"
          width="25"
        >
          <switch>
            <g>
              <path
                fill="#ffb819"
                d="M22.3,34.4c0.1,0.9-0.6,1.6-1.5,1.6h-1.7c-0.9,0-1.6-0.8-1.5-1.6l0.8-8.2
			c0.1-0.7,0.6-1.1,1.2-1.1h0.7c0.6,0,1.2,0.5,1.2,1.1L22.3,34.4z M33.6,25.1l-7.5-3.5c-0.6-0.3-1.3,0-1.6,0.5l-0.4,0.6
			c-0.3,0.6-0.2,1.3,0.4,1.6l6.7,4.8c0.7,0.5,1.7,0.3,2.2-0.5l0.9-1.5C34.7,26.5,34.4,25.5,33.6,25.1z M34.3,12.7l-0.9-1.5
			c-0.4-0.8-1.4-1-2.2-0.5l-6.7,4.8c-0.5,0.4-0.7,1.1-0.4,1.7l0.4,0.6c0.3,0.6,1,0.8,1.6,0.5l7.5-3.5C34.4,14.5,34.7,13.5,34.3,12.7
			z M20.9,4h-1.7c-0.9,0-1.6,0.8-1.5,1.6l0.8,8.2c0.1,0.7,0.6,1.1,1.2,1.1h0.7c0.6,0,1.2-0.5,1.2-1.1l0.8-8.2
			C22.4,4.8,21.7,4,20.9,4z M15.5,15.5l-6.7-4.8C8,10.3,7,10.5,6.6,11.3l-0.9,1.5c-0.4,0.8-0.1,1.7,0.7,2.1l7.5,3.5
			c0.6,0.3,1.3,0,1.6-0.5l0.4-0.6C16.2,16.6,16,15.9,15.5,15.5z M15.9,22.8l-0.4-0.6c-0.3-0.6-1-0.8-1.6-0.5l-7.5,3.5
			c-0.8,0.4-1.1,1.3-0.7,2.1l0.9,1.5c0.4,0.8,1.4,1,2.2,0.5l6.7-4.8C16,24.1,16.2,23.4,15.9,22.8z"
              ></path>
            </g>
          </switch>
        </svg>
      </a>{" "}
      <a
        href={`https://www.samsclub.com/s/${ingredient}`}
        target="_blank"
        rel="noopener noreferrer"
      >
        <img
          src="https://www.samsclub.com/js/common/favicon-v2-32x32.png"
          style={{ height: "15px", width: "15px" }}
        />
      </a>{" "}
      <a
        href={`https://www.meijer.com/shopping/search.html?text=${ingredient}`}
        target="_blank"
        rel="noopener noreferrer"
      >
        <img src="https://www.meijer.com/etc.clientlibs/meijer-spa/clientlibs/clientlib-react/resources/favicon.ico" />
      </a>{" "}
      <a href={`https://www.amazon.com/s?k=${ingredient}`}>
        <svg
          height="15px"
          width="15px"
          version="1.1"
          id="Layer_1"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 291.319 291.319"
          xmlSpace="preserve"
        >
          <g>
            <path
              fill="#F4B459;"
              d="M252.089,239.901c-120.033,57.126-194.528,9.331-242.214-19.7c-2.95-1.83-7.966,0.428-3.614,5.426
		c15.886,19.263,67.95,65.692,135.909,65.692c68.005,0,108.462-37.107,113.523-43.58
		C260.719,241.321,257.169,237.78,252.089,239.901z M285.8,221.284c-3.223-4.197-19.6-4.98-29.906-3.714
		c-10.324,1.229-25.818,7.538-24.471,11.325c0.692,1.42,2.103,0.783,9.195,0.146c7.11-0.71,27.029-3.223,31.18,2.203
		c4.17,5.462-6.354,31.49-8.275,35.687c-1.857,4.197,0.71,5.28,4.197,2.485c3.441-2.795,9.668-10.032,13.847-20.274
		C285.718,238.845,288.249,224.479,285.8,221.284z"
            />
            <path
              fill="#484848;"
              d="M221.71,149.219V53.557C221.71,37.125,205.815,0,148.689,0C91.572,0,61.184,35.696,61.184,67.85
		l47.74,4.27c0,0,10.633-32.136,35.313-32.136s22.987,19.992,22.987,24.316v20.784C135.607,86.149,57.096,95.18,57.096,161.382
		c0,71.191,89.863,74.177,119.332,28.167c1.138,1.866,2.431,3.696,4.051,5.408c10.843,11.398,25.308,24.981,25.308,24.981
		l36.852-36.415C242.658,183.513,221.71,167.071,221.71,149.219z M112.511,152.578c0-30.579,32.764-36.779,54.722-37.507v26.319
		C167.224,193.527,112.511,185.634,112.511,152.578z"
            />
          </g>
        </svg>
      </a>
    </>
  );
}
